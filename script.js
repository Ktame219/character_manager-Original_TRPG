...
function migrateChar(c) {
  ...
  if (c.rerollCount == null) c.rerollCount = 0;
  if (c.age == null)   c.age      = '';
  if (!c.faction)      c.faction  = '';
  if (!c.extraFields)  c.extraFields = [];
  // バトルスキルに新フィールドを補完
  if (c.skills) c.skills = c.skills.map(sk => ({
    name:    sk.name    || '',
    ap:      sk.ap      != null ? sk.ap  : 1,
    sp:      sk.sp      != null ? sk.sp  : 0,
    targets: sk.targets != null ? sk.targets : 1,
    range:   sk.range   || '',
    effect:  sk.effect  || '',
    level:   sk.level   != null ? sk.level : 0,
  }));
...
function makeChar() {
  return {
    id:    uid(),
    name:  '', job: '', gender: '', age: '', faction: '',
    extraFields: [],
    level: 0,
    stats: { str: 10, dex: 10, vit: 10, spi: 10, agi: 10, rei: 10 },
    cur:   { hp: 2, sp: 10, ap: 3, san: 10 },
    talentRolled: false, talentTotal: 0, talentDice: [0, 0], rerollCount: 0, rerollCount: 0,
    abilities:  [],
    origin:     { name: '', effect: '' },
    anomalies:  [],
    skills:     [],
    ...
    <div class="section">
      ...
      <div class="field-grid">
        ...
        <div class="field">
          ...
          <input class="fi" placeholder="—" value="${c.age}" oninput="upd('age',this.value)"/>
        </div>
        <div class="field">
        ...
        <div class="field">
          <div class="field-label">所属勢力</div>
          <input class="fi" placeholder="—" value="${c.faction}" oninput="upd('faction',this.value)"/>
        </div>
      </div>

      <!-- 任意追加項目 -->
      <div style="margin-top:16px">
        <div style="font-size:10px;color:var(--ink-dim);letter-spacing:0.06em;margin-bottom:8px">追加情報</div>
        <div id="extraFieldList">${buildExtraFieldsHTML(c)}</div>
        <button class="btn-ghost" style="margin-top:6px" onclick="addExtraField()">＋ 項目を追加（出身地・髪色・誕生日など）</button>
      </div>
    </div>
      ...
      <div class="talent-box">
        <div style="text-align:center">
          ...
          <button class="btn btn-dice"
            style="margin-top:10px;${c.talentRolled && (c.rerollCount||0) >= 3 ? 'opacity:0.4;cursor:not-allowed' : ''}"
            onclick="doRoll()"
            ${c.talentRolled && (c.rerollCount||0) >= 3 ? 'disabled' : ''}>
            ${!c.talentRolled
              ? '🎲 ロール'
              : (3-(c.rerollCount||0)) > 0
                ? '🎲 振り直し（残り'+(3-(c.rerollCount||0))+'回）'
                : '⛔ 振り直し上限 (3/3)'}
          </button>
        </div>
...
function buildExtraFieldsHTML(c) {
  const PRESETS = ['出身地','髪色','瞳の色','肌の色','誕生日','身長','体重','血液型','利き手','趣味','好きなもの','嫌いなもの'];
  const fields = c.extraFields || [];
  if (!fields.length) return '';
  return fields.map((f, i) => `
    <div class="skill-row" style="margin-bottom:6px">
      <input class="fi" style="width:110px;flex-shrink:0;font-size:12px" placeholder="項目名" value="${f.key}"
        list="extra-presets" oninput="updExtraKey(${i},this.value)"/>
      <datalist id="extra-presets">
        ${PRESETS.map(p => `<option value="${p}">`).join('')}
      </datalist>
      <input class="fi" style="flex:1;font-size:13px" placeholder="内容..." value="${f.val}"
        oninput="updExtraVal(${i},this.value)"/>
      <button class="btn btn-sm btn-d" onclick="rmExtraField(${i})">✕</button>
    </div>`).join('');
}
...
function addExtraField() {
  ...
  if (!c.extraFields) c.extraFields = [];
  c.extraFields.push({ key: '', val: '' });
  redrawExtraFields();
  setSt('');
}

function rmExtraField(i) {
  const c = gc(activeId); if (!c) return;
  c.extraFields.splice(i, 1);
  redrawExtraFields();
  setSt('');
}

function updExtraKey(i, v) {
  const c = gc(activeId); if (!c) return;
  c.extraFields[i].key = v; setSt('');
}

function updExtraVal(i, v) {
  const c = gc(activeId); if (!c) return;
  c.extraFields[i].val = v; setSt('');
}

function redrawExtraFields() {
  const c = gc(activeId); if (!c) return;
  const el = document.getElementById('extraFieldList'); if (!el) return;
  el.innerHTML = buildExtraFieldsHTML(c);
}

function buildSkillHTML(c) {
  const RANGES = ['—', '零距離', '近距離', '中距離', '遠距離', '自由選択'];
  return c.skills.map((sk, i) => `
    <div class="ability-card" style="margin-bottom:0">
      <!-- スキル名・削除 -->
      <div class="ability-head" style="margin-bottom:10px">
        <input class="fi" style="flex:1;font-size:14px;font-weight:500" placeholder="バトルスキル名..." value="${sk.name}" oninput="updSkField(${i},'name',this.value)"/>
      ...
      <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:10px;align-items:center">
        ...
        <div style="display:flex;flex-direction:column;gap:4px;align-items:center">
          <div class="field-label">消費 AP</div>
          <div style="display:flex;align-items:center;gap:4px">
            <button class="sb" style="width:28px;height:28px;font-size:13px" onclick="chgSkNum(${i},'ap',-1,0)">−</button>
            <span style="font-family:var(--font-display);font-size:20px;font-weight:600;min-width:24px;text-align:center;color:var(--accent)">${sk.ap}</span>
            <button class="sb" style="width:28px;height:28px;font-size:13px" onclick="chgSkNum(${i},'ap',1,0)">＋</button>
          </div>
        </div>
        ...
        <!-- 消費SP -->
        <div style="display:flex;flex-direction:column;gap:4px;align-items:center">
          <div class="field-label">消費 SP</div>
          <div style="display:flex;align-items:center;gap:4px">
            <button class="sb" style="width:28px;height:28px;font-size:13px" onclick="chgSkNum(${i},'sp',-1,0)">−</button>
            <span style="font-family:var(--font-display);font-size:20px;font-weight:600;min-width:24px;text-align:center;color:#5b7ec7">${sk.sp}</span>
            <button class="sb" style="width:28px;height:28px;font-size:13px" onclick="chgSkNum(${i},'sp',1,0)">＋</button>
          </div>
        </div>
        ...
        <!-- 指定対象数 -->
        <div style="display:flex;flex-direction:column;gap:4px;align-items:center">
          <div class="field-label">対象数</div>
          <div style="display:flex;align-items:center;gap:4px">
            <button class="sb" style="width:28px;height:28px;font-size:13px" onclick="chgSkNum(${i},'targets',-1,1)">−</button>
            <span style="font-family:var(--font-display);font-size:20px;font-weight:600;min-width:24px;text-align:center;color:var(--ink)">${sk.targets}</span>
            <button class="sb" style="width:28px;height:28px;font-size:13px" onclick="chgSkNum(${i},'targets',1,1)">＋</button>
          </div>
        </div>
        ...
        <div style="display:flex;flex-direction:column;gap:4px;flex:1;min-width:120px">
          <div class="field-label">間合い</div>
          <select class="fi" style="font-size:12px" onchange="updSkField(${i},'range',this.value)">
            ${['—','零距離','近距離','中距離','遠距離','自由選択'].map(r =>
              `<option value="${r}" ${sk.range===r?'selected':''}>${r}</option>`
            ).join('')}
          </select>
        </div>
      ...
      <!-- 詳細効果 -->
      <div class="field">
        <div class="field-label">詳細・効果</div>
        <textarea class="fi" rows="2" placeholder="スキルの効果・条件・特殊ルールなど..." oninput="updSkField(${i},'effect',this.value)">${sk.effect}</textarea>
      </div>
    </div>`).join('');
}
...
function doRoll() {
  const c = gc(activeId); if (!c) return;
  // 初回ロールは無制限、振り直しは3回まで
  if (c.talentRolled && c.rerollCount >= 3) return;
  const d1 = Math.ceil(Math.random() * 100);
  const d2 = Math.ceil(Math.random() * 100);
  if (c.talentRolled) c.rerollCount = (c.rerollCount || 0) + 1;
  c.talentDice   = [d1, d2];
  c.talentTotal  = d1 + d2;
  c.talentRolled = true;
  setSt('');
...
function addSkill() {
  const c = gc(activeId); if (!c) return;
  c.skills.push({ name:'', ap:1, sp:0, targets:1, range:'', effect:'', level:0 }); redrawSkills(); setSt('');
}
function rmSkill(i) {
  const c = gc(activeId); if (!c) return;
...
function updSkField(i, k, v) {
  const c = gc(activeId); if (!c) return;
  c.skills[i][k] = (k === 'ap' || k === 'sp' || k === 'targets') ? (parseInt(v) || 0) : v;
  setSt('');
}
function chgSkNum(i, k, delta, min) {
  const c = gc(activeId); if (!c) return;
  c.skills[i][k] = Math.max(min, (c.skills[i][k] || 0) + delta);
  redrawSkills(); setSt('');
}
  ...
  const skillPreviewHTML = c.skills.length ? c.skills.map(sk => `
    <div style="background:var(--surface3);border:1px solid var(--border2);border-radius:4px;padding:12px 14px;margin-bottom:8px">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;flex-wrap:wrap">
        <span style="font-size:14px;font-weight:500;flex:1">${sk.name || '（名称未定）'}</span>
        <span style="font-size:11px;color:var(--accent);white-space:nowrap">AP: ${sk.ap||0}</span>
        ${(sk.sp||0)>0?`<span style="font-size:11px;color:#5b7ec7;white-space:nowrap">SP: ${sk.sp}</span>`:''}
        <span style="font-size:11px;color:var(--ink-dim);white-space:nowrap">対象: ${sk.targets||1}</span>
        ${sk.range&&sk.range!=='—'?`<span style="font-size:11px;padding:1px 7px;border:1px solid var(--border2);border-radius:2px;color:var(--ink-dim)">${sk.range}</span>`:''}
        <div style="display:flex;gap:3px">${[1,2,3,4,5].map(p =>
          `<div style="width:11px;height:11px;border-radius:50%;background:${sk.level >= p ? '#c0392b' : 'var(--surface4)'};border:1px solid ${sk.level >= p ? '#c0392b' : 'var(--border)'}"></div>`
        ).join('')}</div>
      </div>
      ${sk.effect?`<div style="font-size:12px;color:var(--ink-dim);line-height:1.7;white-space:pre-wrap">${sk.effect}</div>`:''}
    </div>`).join('')
    : `<div style="color:var(--ink-faint);font-size:13px">なし</div>`;
    ...
    <div class="sheet-header">
      <div style="flex:1">
        ...
        <div style="font-size:13px;color:var(--ink-dim);margin-top:4px">
          ${[c.job, c.age ? c.age+'歳' : null, c.gender || null, c.faction || null].filter(Boolean).join(' ・ ') || ''}
        ...
        ${(c.extraFields||[]).length ? `<div style="margin-top:8px;display:flex;flex-wrap:wrap;gap:6px 16px">
          ${(c.extraFields||[]).filter(f=>f.key||f.val).map(f=>`<span style="font-size:12px;color:var(--ink-dim)"><span style="color:var(--ink-faint)">${f.key}：</span>${f.val}</span>`).join('')}
        </div>` : ''}
      </div>
